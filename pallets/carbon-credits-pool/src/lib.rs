    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create(
            ...
            let actual_max_limit = match max_limit {
                Some(limit) => {
                    ensure!(limit <= T::MaxProjectIdList::get(), Error::<T>::MaxLimitGreaterThanPermitted);
                    limit
                }
            ...
            <T as pallet::Config>::AssetHandler::create(id.into(), Self::account_id(), true, (1_u32).into())?;
        ...
        pub fn deposit(
            ...
            Pools::<T>::try_mutate(pool_id, |pool| -> DispatchResultWithPostInfo {
                ...
                let (project_id, group_id) = pallet_carbon_credits::Pallet::<T>
                    ::asset_id_lookup(asset_id)
                    .ok_or(Error::<T>::InvalidPoolId)?;

                // get the details of project
                let project_details: pallet_carbon_credits::ProjectDetail<T> = pallet_carbon_credits::Pallet
                ...
                if let Some(registry_list) = &pool.config.registry_list {
                    ...
                    let project_registry = project_details.registry_details.first().ok_or(Error::<T>::ProjectNotFound)?;
                    ensure!(registry_list.contains(&project_registry.reg_name), Error::<T>::RegistryNotPermitted);
                }

                if let Some(project_id_list) = &pool.config.project_id_list {
                    ensure!(project_id_list.contains(&asset_id), Error::<T>::ProjectIdNotWhitelisted);
                }

                // calculate the issuance year for the project
                let project_issuance_year = pallet_carbon_credits::Pallet
                    ::calculate_issuance_year(project_details, group_id)
                    .ok_or(Error::<T>::ProjectIssuanceYearError)?;

                // transfer the tokens to pallet account
                <T as pallet::Config>::AssetHandler::transfer(asset_id, &who, &Self::account_id(), amount, Protect)?;
                ...
                if let Some(project_map) = issuance_year_map {
                    ...
                    if let Some(existing_amount) = project_details {
                        let new_amount = existing_amount.checked_add(&amount).ok_or(Error::<T>::UnexpectedOverflow)?;

                        project_map.try_insert(asset_id, new_amount).map_err(|_| Error::<T>::UnexpectedOverflow)?;
                    } else {
                        // If the project tokens have been NOT been previoulsy deposited to the
                        // pool, create a new entry
                        project_map.try_insert(asset_id, amount).map_err(|_| Error::<T>::UnexpectedOverflow)?;
                    }
                } else {
        ...
        pub fn retire(
            ...
            Pools::<T>::try_mutate(pool_id, |pool| -> DispatchResultWithPostInfo {
                ...
                <T as pallet::Config>::AssetHandler::burn_from(pool_id.into(), &who, amount, Exact, Polite)?;
                ...
                for (_year, project_map) in pool_credits_temp.iter_mut() {
                    ...
                    for (asset_id, available_amount) in project_map.clone().into_inner().iter_mut() {
                        ...
                        remaining = remaining.checked_sub(&actual).ok_or(Error::<T>::UnexpectedOverflow)?;
                ...
                pool.credits = CreditsMap::<T>
                    ::try_from(pool_credits_temp)
                    .map_err(|_| Error::<T>::UnexpectedOverflow)?;
        ...
        pub fn force_set_pool_storage(origin: OriginFor<T>, pool_id: T::PoolId, data: PoolOf<T>) -> DispatchResult {
            <T as pallet::Config>::ForceOrigin::ensure_origin(origin)?;
