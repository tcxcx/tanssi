...
impl<T: Config> Pallet<T> {
    ...
    pub fn check_kyc_approval(account_id: &T::AccountId) -> DispatchResult {
        if !T::KYCProvider::contains(account_id) { Err(Error::<T>::KYCAuthorisationFailed.into()) } else { Ok(()) }
    }
    ...
    pub fn do_approve_project(project_id: T::ProjectId, is_approved: bool) -> DispatchResult {
        Projects::<T>::try_mutate(project_id, |project| -> DispatchResult {
            ...
            ensure!(project.approved == ProjectApprovalStatus::Pending, Error::<T>::ApprovalAlreadyProcessed);
    ...
    pub fn create_project(
        ...
        Projects::<T>::try_mutate(
            project_id,
            |project| -> Result<T::ProjectId, DispatchError> {
                ...
                for mut group in params.batch_groups.into_iter() {
                    ...
                    for batch in group.batches.iter() {
                        ensure!(batch.total_supply > Zero::zero(), Error::<T>::CannotCreateProjectWithoutCredits);

                        ensure!(batch.minted == Zero::zero(), Error::<T>::CannotCreateProjectWithoutCredits);

                        ensure!(batch.retired == Zero::zero(), Error::<T>::CannotCreateProjectWithoutCredits);
                    ...
                    ensure!(group_total_supply > Zero::zero(), Error::<T>::CannotCreateProjectWithoutCredits);
                    ...
                    batch_group_map.try_insert(group_id, group.clone()).map_err(|_| Error::<T>::TooManyGroups)?;
    ...
    pub fn do_add_batch_group(
        ...
        Projects::<T>::try_mutate(project_id, |project| -> DispatchResult {
            ...
            for batch in batch_group.batches.iter() {
                ...
                group_total_supply = group_total_supply.checked_add(&batch.total_supply).ok_or(Error::<T>::Overflow)?;
            }
            ...
            // insert the group to BTreeMap
            batch_group_map.try_insert(group_id.into(), batch_group.clone()).map_err(|_| Error::<T>::TooManyGroups)?;
    ...
    pub fn mint_carbon_credits(
        ...
        Projects::<T>::try_mutate(project_id, |project| -> DispatchResult {
            ...
            let projected_total_supply = amount_to_mint.checked_add(&group.minted).ok_or(Error::<T>::Overflow)?;

            ensure!(projected_total_supply <= group.total_supply, Error::<T>::AmountGreaterThanSupply);
            ...
            for batch in batch_list.iter_mut() {
                ...
                let available_to_mint = batch.total_supply.checked_sub(&batch.minted).ok_or(Error::<T>::Overflow)?;
    ...
    pub fn retire_carbon_credits(
        ...
        Projects::<T>::try_mutate(project_id, |project| -> DispatchResult {
            ...
            group.retired = group.retired.checked_add(&amount).ok_or(Error::<T>::AmountGreaterThanSupply)?;
            ...
            for batch in batch_list.iter_mut() {
                ...
                let available_to_retire = batch.minted.checked_sub(&batch.retired).ok_or(Error::<T>::Overflow)?;
                ...
                // add to retired list
                batch_retire_data_list.try_push(batch_retire_data).map_err(|_| Error::<T>::Overflow)?;
            ...
            let item_id = match maybe_item_id {
                None => {
                    ...
                    T::NFTHandler::create_collection(&group.asset_id, &Self::account_id(), &Self::account_id())?;
                    Default::default()
                }
                Some(x) => x,
            };
            ...
            let next_item_id: T::ItemId = item_id.checked_add(&One::one()).ok_or(Error::<T>::Overflow)?;
