...
pub mod pallet {
    ...
    pub type AuthorizedAccounts<T: Config> = StorageValue<_, AuthorizedAccountsListOf<T>, ValueQuery>;

    #[pallet::storage]
    #[pallet::getter(fn projects)]
    /// The details of a CarbonCredits
    pub(super) type Projects<T: Config> = StorageMap<_, Blake2_128Concat, T::ProjectId, ProjectDetail<T>>;

    #[pallet::storage]
    #[pallet::getter(fn asset_id_lookup)]
    /// AssetId details for project/group
    pub(super) type AssetIdLookup<T: Config> = StorageMap<_, Blake2_128Concat, T::AssetId, (T::ProjectId, T::GroupId)>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn approve_project(origin: OriginFor<T>, project_id: T::ProjectId, is_approved: bool) -> DispatchResult {
        ...
        pub fn mint(
            ...
            // Self::check_kyc_approval(&sender)?;
            Self::mint_carbon_credits(sender, project_id, group_id, amount_to_mint, list_to_marketplace)
        }
        ...
        pub fn force_add_authorized_account(origin: OriginFor<T>, account_id: T::AccountId) -> DispatchResult {
            T::ForceOrigin::ensure_origin(origin)?;
            // add the account_id to the list of authorized accounts
            AuthorizedAccounts::<T>::try_mutate(|account_list| -> DispatchResult {
                ensure!(!account_list.contains(&account_id), Error::<T>::AuthorizedAccountAlreadyExists);

                account_list.try_push(account_id.clone()).map_err(|_| Error::<T>::TooManyAuthorizedAccounts)?;
                Ok(())
            })?;
        ...
        pub fn force_remove_authorized_account(origin: OriginFor<T>, account_id: T::AccountId) -> DispatchResult {
            T::ForceOrigin::ensure_origin(origin)?;
        ...
        #[pallet::call_index(9)]
        #[pallet::weight(T::WeightInfo::force_set_next_item_id())]
        pub fn force_set_next_asset_id(origin: OriginFor<T>, asset_id: T::AssetId) -> DispatchResult {
            T::ForceOrigin::ensure_origin(origin)?;
        ...
        #[pallet::weight(T::WeightInfo::force_set_project_storage())]
        pub fn force_remove_project(origin: OriginFor<T>, project_id: T::ProjectId) -> DispatchResult {
            T::ForceOrigin::ensure_origin(origin)?;
